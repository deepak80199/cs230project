...
def get_data_numpy():
    ...
    X_train_new = np.concatenate((X_train_new,np.vectorize(add_rand)(x_train)),axis= 0)
    X_train_new = np.concatenate((X_train_new,np.vectorize(sub_rand)(x_train)), axis=0)
    y_train_new  = y_train
    y_train_new = np.concatenate((y_train_new,y_train), axis= 0)
    y_train_new = np.concatenate((y_train_new,y_train), axis= 0)
    
    # preprocessing
    X_train, Y_train, X_test, Y_test = preprocess_numpy(x_train,y_train,x_test,y_test)
